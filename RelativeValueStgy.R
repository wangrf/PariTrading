...
Cointegration <- function(x) {
  vals<-data.frame(x)
  beta<-ifelse(class(try(coef(lm(vals[,1]~vals[,2]+0,data=vals))[1],TRUE))=="try-error",NA,coef(lm(vals[,1]~vals[,2]+0,data=vals))[1])
  ifelse(is.na(beta),NA, (adf.test(vals[,1]-beta*vals[,2], alternative="stationary", k=0))$p.value)
...
dtwDistX<-function(x){
  ...
  xx<-t(x)
  ifelse(class(try(dtwDist(xx)[1,2],TRUE))=="try-error",NA,dtwDist(xx)[1,2])
...
cumReturn<-merge_ind(symbols = sym50,ind = paste0("cumReturn", return.nDay, "D"))
